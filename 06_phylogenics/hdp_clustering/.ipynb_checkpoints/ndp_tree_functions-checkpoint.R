...
infer.tree.branches <- function(clust_pairs, centiles_tbl_all, min_contrib = 0.10) {
  ...
  for(i in 1:dim(clust_pairs)[1]){
    ...
    if(all(sign(centiles_tbl_all[idx,idx.cl1] - centiles_tbl_all[idx,idx.cl2]) == 1)){
      ...
      x.branches = rbind(x.branches,c('PD',unique(sort(centiles_tbl_all$cl1[idx])),'strong'))
      x.branches = rbind(x.branches,c('PD',unique(sort(centiles_tbl_all$cl2[idx])),'strong'))
    }
  }
  ...
  for(i in 1:length(idx)){
    x.branches = rbind(x.branches,c('PD',as.character(unique(x.branches[,'From'])[idx[i]]),'strong'))
  }
  ...
  for(i in 1:length(idx)){
    x.branches = rbind(x.branches,c('PD',rownames(x.cluster_and_samples.final)[idx[i]],'strong'))
  }
  x.branches = unique(x.branches)
