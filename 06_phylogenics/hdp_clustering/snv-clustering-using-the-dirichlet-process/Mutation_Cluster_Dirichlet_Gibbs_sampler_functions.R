...
heatmap.iter <- function(gs.out, curr.iter, min.threshold = 10, samp.names) {
  ...
  # to handle case of only 1 column, can't plot heatmap
  if(length(alloc.counts) == 1){alloc.counts = c(alloc.counts,alloc.counts)}
  ...
  if(length(as.dist(1 - distance(matrix(c(temp.r), ncol=ncol(temp.r)) / rowSums((temp.r)), method="cosine"))) != 0){

   temp.dendro <- as.dendrogram(hclust(as.dist(1 - distance(matrix(c(temp.r), ncol=ncol(temp.r)) / rowSums((temp.r)), method="cosine"))))
...
# this function is also in Mutation_Cluster_Dirichlet_posthoc_merge_fx.R (comment out)
if(FALSE){
...
merge.clusters.differ.by.seq.error <- function(gs.out, ls.out, min.true.VAF = 0.025, overlap.frac=0.025, min.num.muts = 20, ls.type="ECR", burn.in, version=2) {
  ...
  for (i in 1:(length(unique.clusters)-1)) {
    ...
    # currently merge step skipped if N/A values detected
