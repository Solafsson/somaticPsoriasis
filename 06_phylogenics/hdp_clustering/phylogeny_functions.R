...
convert_edges_to_phylo <- function(tree_tbl, muts_per_cluster) {
  ...
  # Define root node
  donor_id = unique(tree_tbl$From[grepl('P', tree_tbl$From)])
...
draw_nice_tree <- function(tr, show_internal_nodes=FALSE, edge_strength=NA, patientID=id.patient,...) {
  if(is.na(edge_strength)) {
    plot(tr, ...)#, show.node.label=TRUE)#, direction='downwards')
  } else {
    col_veryweak = '#000000'
    col_weak = '#000000'# '#c1c1c1'
    col_med= '#000000'
    col_strong = '#000000'
    edge_col = as.character(factor(edge_strength, levels=c('strong', 'medium', 'weak','very weak'), labels=c(col_strong, col_med, col_weak, col_veryweak)))
...
highlight_tree_nodes <- function(tr, pch=15, show_internal_nodes=FALSE, ...) {
  ...
  if(show_internal_nodes) {
    ...
    for(i in c(1:length(x))) {
      a=legend(x[i],y[i],str_replace(tr$node.label[i], 'Cl.', ''), box.col = "black", bg = "grey", adj = 0, cex=0.5, xjust=0.5, yjust=0.5, plot=FALSE)
      ...
      rect(xleft=a$left, ytop=mid+0.5*reduction*a$h, xright=a$left+a$w, ybottom=mid-0.5*reduction*a$h, bty='n', col = "black", border = "grey")
      text(x[i]-0.2*a$w,y[i],str_replace(tr$node.label[i], 'Cl.', ''), adj = 0, cex=0.5, xjust=0.5, yjust=0.5,col='white')
