...
# while read sample; do awk -v SAM="$sample" '$1=="Human_exome_v5_Covered" {print SAM, $22, $10,$19}' < ${sample}.all.hsmetrics >> coverage_report.txt; done < <( awk '$8=="WES" && ($16=="NA" || $16=="Technical_duplicate") {print $1}' < /nfs/users/nfs_s/so11/phd/psoriasis/bsub_jupyter_lab/psoriasis/sample_info/sample_meta.txt )
