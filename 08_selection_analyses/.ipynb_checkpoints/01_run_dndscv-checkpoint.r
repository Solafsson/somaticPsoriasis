...
d38_lesional = dndscv(lesional_mutations, refdb=refcds_38, cv=scores,max_muts_per_gene_per_sample = Inf, max_coding_muts_per_sample = Inf) 

sel_cv_lesional = d38_lesional$sel_cv
signif_genes_lesional = sel_cv_lesional[sel_cv_lesional$qglobal_cv<0.05 | sel_cv_lesional$qallsubs_cv<0.05, c(1:6, 17,19)]
signif_genes_lesional


## Explore the drivers that likely result from PUVA exposure
## Need to formalize that analysis. 
puva_muts <- mutations_in_signif_genes[mutations_in_signif_genes$ref3_cod %in% c("ATA", "CTA", "GTA", "TTA"),]




## Have a look at the "driver preference" 
# Note: Will break once I reach patients >99
mutations_in_signif_genes$Patient <- paste("patient", substr(mutations_in_signif_genes$sampleID, 2,3), sep="")

by_patient <- data.frame(table(mutations_in_signif_genes$Patient[mutations_in_signif_genes$impact!="Synonymous"], mutations_in_signif_genes$gene[mutations_in_signif_genes$impact!="Synonymous"]))
colnames(by_patient) <- c("Patient", "Gene", "Mutations")


ggplot(by_patient, aes(x=Patient, y=Gene)) + theme_classic() +
  geom_tile(aes(fill=Mutations)) + theme(axis.text.x = element_text(angle=90)) + labs(x="", y="") + 
  geom_text(aes(label = Mutations)) + 
  scale_fill_gradient(low = "white", high = "red") 

patient_meta <- read.table("/Users/so11/phd/psoriasis/sample_info/patient_meta.txt", h=T, stringsAsFactors = F)
by_patient <- merge(by_patient, patient_meta, by.x="Patient", by.y="Patient.ID")
